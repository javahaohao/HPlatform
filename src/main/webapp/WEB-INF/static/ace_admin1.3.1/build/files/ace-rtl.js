	  ...
	  if(selector.match(/@media/i) || selector.match(/@(?:-(?:moz|webkit|ms|o|khtml)-)?keyframes/i)) {
		...
		pre_media_length = rtl_output.length;
			...
			for(var c = 0 ; c < selector_parts.length; c++) {
				...
				if(selector_parts[c].match(/(?:(?:[_\-\.])(?:left|right))|(?:\.arrowed)|(?:\.input-icon)|(\.rtl)|(\-rtl)|(\.dropdown-menu[^\:])|(\.scroll-)|(\.ace-switch)|(\.lbl)|(\.popover)|(\.table)|(\.ui-slider)|(\.nav-tabs)|(blockquote)|(\.datepicker)|(\.daterangepicker)|(\.dropdown-menu-right)|(\.footer-)|(\.chosen-)/i)) {
					continue;
				}
