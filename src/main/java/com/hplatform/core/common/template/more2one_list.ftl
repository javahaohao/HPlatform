					    ...
					    <tbody>
					        <c:forEach items="${"$"}{${table.domainName?uncap_first}List}" var="${table.domainName?uncap_first}" varStatus="stat">
					            <tr>
					            	...
					            	<#list table.parent.columnList as column> 
					            	<#if !column.hideFlag??||!column.hideFlag>
						            <td>${"$"}{${table.domainName?uncap_first}.${"$"}{table.parent.domainName?uncap_first}}.${column.propertiesName}}</td>
						            </#if>
						            </#list>
					            	<#list table.columnList as column> 
					            	<#if !column.hideFlag??||!column.hideFlag>
	...
	<script type="text/javascript">
		$(function(){
			...
			$('#sortable').dataTable({
				"aoColumns": [
			      { "bSortable": false },
			      { "bSortable": false },<#list table.parent.columnList as column><#if !column.hideFlag??||!column.hideFlag><#if !column.sortFlag??||!column.sortFlag>{ "bSortable": false },<#else>null,</#if></#if></#list> <#list table.columnList as column><#if !column.hideFlag??||!column.hideFlag><#if !column.sortFlag??||!column.sortFlag>{ "bSortable": false },<#else>null,</#if></#if></#list>
				  { "bSortable": false }
				],"aaSorting": []});
			//添加表格全选事件
			bindCheckAllEvent();
